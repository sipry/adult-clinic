...
import brandArrow from "@/../public/assets/svg/brand-arrow.svg";
import patient from "@/../public/assets/svg/patient.svg";

/* ---------- Card images ---------- */
import baby from "@/../public/assets/images/baby.jpg";
import covid from "@/../public/assets/images/covid.jpg";
import education from "@/../public/assets/images/edu.jpg";
import obesity from "@/../public/assets/images/obesity.jpg";
import urgent from "@/../public/assets/images/urgent.jpg";
import asthmaImg from "@/../public/assets/images/asthma.jpg";
import adolecent from "@/../public/assets/images/adolecent.jpg";
...
/* ---------- Icon color palette (no repetidos consecutivos) ---------- */
const ICON_COLORS = ["#C2CC00", "#7A0062", "#264660", "#FFD200", "#CC1616", "#C2CC00"] as const;

/* --- Transparencia y contraste para los íconos --- */
...
/**
 * Calcula el color del ícono que mejor contrasta con el fondo del círculo.
 * Mezcla el color de fondo con "blanco" considerando la transparencia,
 * y decide entre blanco (#fff) o slate-800 (#1f2937).
 */
const getReadableTextColor = (hex: string, alpha = 0.85) => {
  const { r, g, b } = hexToRgb(hex);
  // fondo real aproximado (blanco) después de aplicar alpha
  ...
  return luminance > 0.6 ? "#1f2937" : "#ffffff"; // texto oscuro si fondo es claro, y viceversa
};


const getIconColor = (index: number) => {
  // Evita repetidos consecutivos, incluso si la paleta tiene duplicados no adyacentes.
...
const iconTextColor = (hex: string) => {
  ...
  const luminance = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
  return luminance > 0.6 ? "#1f2937" /* slate-800 */ : "#ffffff";
};
...
const ServicesShowcase: React.FC = () => {
  ...
  // Esperamos un array tipo: { key: string; title: string; description: string }
  const i18nServices = tArray<{ key?: string; title?: string; description?: string }>(
    "services.list"
  );

  // === Mapeo de íconos por key (incluye variantes con y sin guiones)
  const ICON_BY_KEY: Partial<Record<string, IconKey>> = {
    // nuevos (coinciden con tus keys de translations)
    "well": "baby",
    "well-visit": "baby",
  ...
  // === Mapeo de imágenes por key (incluye variantes con y sin guiones)
  const IMAGE_BY_KEY: Partial<Record<string, string>> = {
    // nuevos
    "well": baby.src,
    "well-visit": baby.src,

    "sick": urgent.src,
    "sick-visit": urgent.src,

    "followup": urgent.src,
    "follow-up": urgent.src,

    "immunizations": covid.src,

    "food-allergy": education.src,
    "food-allergy-test": education.src,

    "environmental-allergy": asthmaImg.src,
    "environmental-allergy-test": asthmaImg.src,

    "vision": adolecent.src,
    "vision-screening": adolecent.src,

    "audiology": education.src,
    "audiology-screening": education.src,

    "obesity": obesity.src,
    "obesity-care-plan": obesity.src,

    "asthma": asthmaImg.src,
    "asthma-care-plan": asthmaImg.src,

    "adhd": education.src,
    "adhd-care-plan": education.src,

    // legacy por compatibilidad
    "adolescent": adolecent.src,
    "urgent": urgent.src,
    "physical": adolecent.src,
    "covid": covid.src,
  };

  // Construimos Services desde i18n si hay elementos, si no fallback
  const servicesFromI18n: Service[] = i18nServices.map((s, idx) => {
    const key = (s.key || `svc-${idx}`).toLowerCase();
    const icon = ICON_BY_KEY[key] ?? "heart";
    return {
      id: key,
      icon,
  ...
  // Solo 1 tarjeta en pantallas pequeñas: peek = 0
  ...
  const THRESHOLD_PX = 60; // desplazamiento mínimo para cambiar
  const MAX_DRAG_PX = 140; // límite visual del arrastre
  const MIN_VELOCITY = 0.35; // px/ms para "flick"
  const TAP_MAX_MOVE = 8; // px

  useEffect(() => {
    // al cambiar de tarjeta, vuelve al frente
    setFlipped(false);
  }, [activeSafe]);
  ...
  const onPointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    if (e.button && e.button !== 0) return; // sólo primario
    startXRef.current = e.clientX;
    startYRef.current = e.clientY;
    startTRef.current = performance.now();
    startTargetRef.current = e.target;

    // ¿el toque inició dentro de la tarjeta activa?
  ...
  const endDrag = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    const v = absDx / dt; // px/ms

    const isTap = absDx < TAP_MAX_MOVE && absDy < TAP_MAX_MOVE;

    // TAP para voltear (sólo si empezó dentro de la tarjeta y no fue sobre un control)
    ...
    if (dragging && shouldSwitch) {
      const dir: 1 | -1 = dx < 0 ? 1 : -1; // izquierda -> siguiente
      setDragX(0);
      setDragging(false);
      go(dir);
    } else {
  ...
  return (
    ...
    >
      ...
      <img
        ...
        alt=""
        aria-hidden
        className="pointer-events-none absolute -left-2 -top-18 lg:block w-[250px] md:w-[300px] xl:w-[380px] opacity-10"
      />
      <img
        src={brandArrow.src}
        alt=""
        aria-hidden
        className="pointer-events-none absolute -left-6 -bottom-5 hidden md:block w-[350px] opacity-10 z-0"
      />
      ...
      <div className="relative max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-start">
        ...
        <div className="max-w-xl lg:self-center mx-auto text-center lg:text-left">
          ...
          {/* Arrows + soft "See all our Services" */}
          <Reveal y={10} delay={260}>
            <div className="mt-8 flex items-center gap-2 justify-center lg:justify-start">
              <button
                ...
                onClick={() => go(-1)}
                aria-label="Anterior"
                className="inline-flex h-10 w-20 lg:w-30 items-center justify-center rounded-sm bg-sky-100 text-slate-800 shadow-sm hover:bg-sky-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500"
              >
              ...
              <Link
                href="/services"
                className="ml-1 h-10 inline-flex items-center gap-2 rounded-sm bg-sky-900 px-4 py-2.5 text-xs md:text-sm font-semibold text-white shadow-sm hover:bg-sky-800 whitespace-nowrap"
              >
        ...
        >
          ...
          <img
            ...
            className="
              pointer-events-none
              absolute -top-48 left-10 -translate-x-1/2
              w-80 md:w-90
              opacity-10
              rotate-0
              z-0
                    ...
                    >
                      <div className="group h-full w-full [perspective:1200px]">
                        <article
                          ...
                          // flip controlado por estado (tap)
                        ...
                        >
                          ...
                          <div className="absolute inset-0 [backface-visibility:hidden] flex flex-col items-center justify-center text-center p-8 md:p-10">
                            <div
                             className={[
                                ...
                                isCenter ? "bg-sky-900 text-white" : "bg-sky-100 text-sky-900",
                              ].join(" ")}
                            >
                          ...
                          {/* Back face */}
                          <div className="absolute inset-0 rounded-[28px] [transform:rotateY(180deg)] [backface-visibility:hidden] overflow-hidden">
                            {s.image ? (
                              <>
                                <img
                                  src={s.image}
                              ...
                              <div className="absolute inset-0 bg-[#1B3B8A]" />
                            )}

                            <div className="relative z-10 h-full w-full flex flex-col items-center justify-center text-center px-8 md:px-10">
                              <h4 className="text-white font-extrabold text-2xl leading-tight mb-4">
                                {s.title}
                              </h4>
                              <p className="text-white/95 text-lg leading-relaxed max-w-[30ch] mb-6">
                              ...
                              <a
                                href={s.href || "#"}
                                className={[
                                  "inline-flex items-center justify-center rounded-sm px-5 py-2.5 text-sm font-semibold shadow-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40",
                                  s.image
                                    ? "text-gray-800 bg-white hover:bg-white/90"
                                    : "text-white bg-[#456DC7] hover:bg-[#3f64b6]",
                                ].join(" ")}
                              >
                                {t("services.details") || "View Details"}
                              </a>
      ...
      {/* CSS vars */}
      <style>{`
        /* Por defecto (móvil / tablet): una tarjeta y más ancho con bleed */
        .svc-wrap { --peek: 0px; --card-h: 340px; }
        /* Hacemos la pista un poco más ancha que el contenedor para "more width" */
        @media (max-width: 1023.98px) {
          .svc-peek { margin-left: -12px; width: calc(100% + 24px); }
        }
        /* En desktop mostramos el peek de la segunda tarjeta */
